...
$.fn.help_bubbles_paginator = function(options) {
	...
	// removing the spotlight elements if they are not existing on the DOM
	for(var i=0; i<o.target_list.length; i++){
		if( $(o.target_list[i][0]).length == 0 ){
			o.target_list.splice(i, 1);
		}
	}
	...
	if( o.closeOnOverlayClick ){
		o.$overlay.one('click.help_bubbles', close);
	}
	o.$overlay.fadeIn();
	...
	function close(){
		...
		$.event.trigger('close_help_bubble'); // trigger a generic event - to handle the close event
	...
	function removeCurrentBubble() {
		if(o.component){
			...
			if( o.component[1][0].hasOwnProperty('altSelector') ){
				$(o.component[1][0]['altSelector']).removeClass("target_selected");
			}
		}
	...
	function getTarget(){
		...
		if( !$target.is(":visible") && o.component[1][0].hasOwnProperty('altSelector') ){
			$target = $( o.component[1][0]['altSelector'] );
		}
