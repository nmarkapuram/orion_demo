 ...
 $.fn.splitter = function(args){
	...
	return this.each(function() {
		...
		function resize_auto_fired() {
  			...
  			return ($.browser.msie && (parseInt($.browser.version) < 8)); //We may need to revert back to 9 version
		}
		...
		function doSplitMouse(evt) {
			...
			if ( opts.outline ) {
				...
				bar._DA = (opts.barWidth) ?  bar[0][opts.pxSplit] : 0;
			} else 
				resplit(pos);
		...
		function resplit(pos, flg) {
			bar._DA = (opts.barWidth) ? bar[0][opts.pxSplit] : 0;  // bar size may change during dock
			...
			if ( (opts.dockPane == A && pos < Math.max(A._min, bar._DA)) ||
				...
				if(!flg) 
					...
					if(!isNaN(A._init)){
						pos = parseInt((A._init - ((opts.sWidth - splitter._DA) * (A._init/opts.sWidth))),10);
						A._min = parseInt(pos * (A._min/A._init),10);
						//A._max = pos + parseInt((splitter._DA - pos)* ((opts.sWidth - A._init)/opts.sWidth),10);
						A._max = parseInt(pos * (A._max/A._init),10);
						B._max = splitter._DA - A._min;
						B._min = splitter._DA - A._max;
						A._init=pos;
						opts.sWidth = splitter._DA;
						opts.pageWidth = $(window).width();
					} else if (!isNaN(B._init)){
						pos = parseInt((B._init - ((opts.sWidth - splitter._DA) * (B._init/opts.sWidth))),10);
						B._min = parseInt(pos * (B._min/B._init),10);
						//B._max = pos + parseInt((splitter._DA - pos)* ((opts.sWidth - B._init)/opts.sWidth),10);
						B._max = parseInt(pos * (B._max/B._init),10);
						//A._max = splitter._DA - B._min;
						//A._min = splitter._DA - B._max;
						B._init= pos;
						pos = splitter._DA - pos;
						opts.sWidth = splitter._DA;
						opts.pageWidth = $(window).width();
					} else {
						pos = Math.max(A._min, splitter._DA - B._max, 
			...
			splitter._ht = $(splitter).find('header').outerHeight();
			if(opts.type === "v"){
				bar.css(opts.origin, pos).css(opts.fixed,(splitter._DF - splitter._ht)).css('top',splitter._ht);
			} else if(opts.type === "h"){
				bar.css(opts.origin, pos).css(opts.fixed,splitter._DF);
			}
			A.css(opts.origin, 0).css(opts.split, pos).css(opts.fixed,  splitter._DF);
			B.css(opts.origin, pos)
				.css(opts.split, splitter._DA-bar._DA-pos).css(opts.fixed,  splitter._DF);
			// IE fires resize for us; all others pay cash
			if ( !resize_auto_fired() ){
				panes.trigger("resize");}
			else{//This is very specific to IE7 , this methoed need to be there in calling page (e.g. Users & permissions and licenses page)
				if(adjustHtForIE7 &&adjustHtForIE7!=null){
					adjustHtForIE7();
				}
			}	
		}
		...
		var panes = $(".splitterChild", splitter[0]).addClass(opts.paneClass).css({
		...
		splitter
			...
			.bind("resize"+opts.eventNamespace, function(e, size){
				...
				if(opts.sWidth==0) opts.sWidth= splitter._DA;
				// Bail if splitter isn't visible or content isn't there yet
				if ( splitter._DF <= 0 || splitter._DA <= 0 ) return;
				// Re-divvy the adjustable dimension; maintain size of the preferred pane
				resplit(!isNaN(size)? size : (!(opts.sizeRight||opts.sizeBottom)? A[0][opts.pxSplit] :
					splitter._DA-B[0][opts.pxSplit]-bar._DA), (opts.sWidth!=splitter._DA && opts.pageWidth!=$(window).width()));
				setBarState(opts.barNormalClass);
			})
