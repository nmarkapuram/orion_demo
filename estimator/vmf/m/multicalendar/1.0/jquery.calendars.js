...
$.extend(CalendarsPicker.prototype, {
	...
	defaultRenderer: {
		...
		picker: '<div class="calendars"><div><div class="calLabelFrm">From</div><div class="divTo"><div class="labTo">To</div><div class="closeBtn">{link:close}</div></div></div>' +
		...
		'<div class="calendars-clear-fix"></div><div class="footCal"> </div></div>',
	...
	_disablePlugin: function(target) {
		...
		if (inst.inline) {
			...
			target.attr('disabled', true); //changed from prop to attr function
			inst.trigger.filter('button.' + this._triggerClass).
	...
	_generateContent: function(target, inst) {
		...
		picker.find(inst.options.renderer.daySelector + ' a').hover(
			...
			find('select.' + this._monthYearClass + ':not(.' + this._anyYearClass + ')').change(function() {
				...
				if($(this).closest('div.calendars-month').hasClass('last')){
					monthYear[0] = monthYear[0] - 1;
					if (monthYear[0] == 0) { monthYear[0] = 12; monthYear[1] = monthYear[1] - 1;}
				}
	...
	_generateMonth: function(target, inst, year, month, calendar, renderer, first) {
		...
		monthHeader = (true ? this._generateMonthSelection(
...
/******************************************************** integrate with vmf framework ****************/

vmf.calendarsPicker = function($){
	return {
		start: function(obj, options){
			obj.calendarsPicker(options)
		}
	};
}(jQuery);