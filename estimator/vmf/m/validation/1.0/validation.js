...
$.extend($.validator, {
	...
	prototype: {
		...
		showLabel: function(element, message) {
            ...
            if (vmf && vmf.tooltip) {
                vmf.tooltip.build(vmf.dom.get(".warning-tip"), {
                    splitTitle: '|',
                    dropShadow: false,
                    showTitle: false
                });
                vmf.tooltip.build(vmf.dom.get(".error-tip"), {
                    splitTitle: '|',
                    dropShadow: false,
                    showTitle: false
                });
...
vmf.validation = function($){
    return {
        build: function(formId, config){
            $("#" + formId).validate(config);
        },
        format: function(str){
            return $.format(str);
        },
        valid: function(elems){
            return (elems instanceof jQuery ? elems : jQuery(elems)).valid();
        },
        removeRules: function(elems, rules){
            (elems instanceof jQuery ? elems : jQuery(elems)).rules("remove", rules);
        }
    }
}(jQuery);
