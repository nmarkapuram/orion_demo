...
$.fn.dropdownReplacement = function(opts, operator){
  ...
  var detectCharWidth = function(testText){
	...
	var setSelection = function(text){
		...
		if(!$selectedOption.hasClass(opts.isSelectable))
			grpName = $selectedOption.prevAll("div.optionGroup:first").text();
		
		setSelectText($selectedSelect, text, grpName);
	...
	var setSelectText = function($select, text, grpName){
	...
	var constructOptions = function($select){ //this is done once: order #1
		...
		var constructWithJSONOptions = function(){
			...
			for(var i = 0; i < l.length; i++){
				...
				if(l[i].lbl=="true" && l[i].isSelectable == "false"){//adding option groups
					var $option = $("<div>",{text:l[i].t,"class":(opts.showBdr)?(opts.optionGroupBdrClass+' '+opts.optionGroupClass):opts.optionGroupClass});
					opts.optionGroup = true;
					opts.showBdr = true;
				}else if(l[i].lbl=="true" && l[i].isSelectable == "true"){
					var $option = $("<a>", {
						href:"#",
						name: l[i].v,
						text: l[i].t,
						title:l[i].f,
						"class":(opts.optionGroup)?opts.optionClass + " " + opts.optionClassExtra + " " + opts.isSelectable :opts.optionClass
					});
				}else {
					var $option = $("<a>", {
						href:"#",
		...
		var constructWithNativeSelect = function(){
			...
			for(var i = 0; i < l.length; i++){
				...
				opts.options[i] = {"t":text, "v":value, "c":className,"f":text,"lbl":(opts.optionGroup)?$option.attr('label'):false, "isSelectable":false}; //f is to display title
				if(opts.options[i].lbl == "true"){
					opts.options[i].isSelectable = $option.attr('isSelectable');
				}else{
					opts.options[i].isSelectable = false;
				}
				if(text.length>opts.inputMaxLength)
					text=text.substring(0,opts.inputMaxLength)+" ...";
	...
	var init = function(){
		...
		$self.each(function(index){
			...
			var $selOpt = $options.find('a[name="'+selectDefaultSelection+'"]');
			var labelName="";
			if (typeof $selOpt!="undefined"){
				labelName = $selOpt.prevAll("div.optionGroup:first").text();
			}
			
			if (labelName!="") 
				setSelectText($select, selectDefaultSelection, labelName);
			else				
