			...
			this.fnServerData = function ( url, data, callback, settings, type, errorCb) {
				settings.jqXHR = $.ajax( {
					...
					"success": function (json) {
						...
						if(  settings.sAjaxDataProp !== "" && json.hasOwnProperty(settings.sAjaxDataProp) ){
							callback( json );
						}else if (typeof res.aaData != "object" || res.aaData==null){
							_fnProcessingDisplay( settings, false );
							if (settings.error) settings.error(settings.oInstance, json, settings);
							else callback( json );
							return;
						} else callback( res );
					},
					...
					"error": function (xhr, error, thrown) {
						...
						if(settings.error) settings.error(settings.oInstance, {"ERROR_CODE":500,"ERROR_MESSAGE":settings.errMsg || "The system encountered a problem. Please try again later."}, settings);
...
$.fn.dataTableExt.oApi.fnReloadAjax = function ( oSettings, sNewSource, fnCallback, method, data, errorCb, bStandingRedraw )
{
	...
	oSettings.fnServerData( oSettings.sAjaxSource, data, function(json) {
		...
		}else if( oSettings.sAjaxDataProp !== "" && json.hasOwnProperty(oSettings.sAjaxDataProp) ){
			var aData =  that.oApi._fnGetObjectDataFn( oSettings.sAjaxDataProp )( json );
